...
function App() {
  ...
  const roomCards = [
    ...
    ...getAllRooms().map((room) => ({
      id: room.id,
      name: room.name,
      icon: room.kind === 'user-created' ? '🔥' : { 'downtown-hub': '🏙️', 'forest-gate': '🌲', 'sunset-temple': '⛩️' }[room.id] || '🏛️',
      ...
      accent: room.kind === 'user-created' ? '#f97316' : { 'downtown-hub': '#60a5fa', 'forest-gate': '#4ade80', 'sunset-temple': '#f472b6' }[room.id] || '#a78bfa',
      bg: room.kind === 'user-created'
        ? 'linear-gradient(135deg, #1a0e00 0%, #0f172a 100%)'
        : { 'downtown-hub': 'linear-gradient(135deg, #0a1628 0%, #0f172a 100%)', 'forest-gate': 'linear-gradient(135deg, #0a1a0e 0%, #0f172a 100%)', 'sunset-temple': 'linear-gradient(135deg, #1a0a14 0%, #0f172a 100%)' }[room.id] || 'linear-gradient(135deg, #120a1a 0%, #0f172a 100%)',
    }))
  ];
